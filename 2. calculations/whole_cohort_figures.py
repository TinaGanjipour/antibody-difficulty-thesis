        ...
        if np.all(np.isclose(x, x.astype(int))) and len(np.unique(x.astype(int))) <= 30:
            ...
            for xv in xs:
                ...
                if mask.sum() < 10:
                    continue
                yy = y[mask]
                q_rows.append((float(xv), float(np.quantile(yy, 0.10)), float(np.quantile(yy, 0.50)), float(np.quantile(yy, 0.90))))
            if q_rows:
            ...
            for b0, b1 in zip(bins[:-1], bins[1:]):
                ...
                if mask.sum() < 25:
                    continue
                yy = y[mask]
                q_rows.append(((b0 + b1) / 2.0, float(np.quantile(yy, 0.10)), float(np.quantile(yy, 0.50)), float(np.quantile(yy, 0.90))))
            if q_rows:
...
def summarize_feature_stats(
    ...
    for c in numeric_features + categorical_features + ["h3_rmsd"]:
        ...
        miss_rows.append({"col": c, "n_present": int(len(s) - n_missing), "n_missing": n_missing, "frac_missing": n_missing / max(1, len(s))})
    pd.DataFrame(miss_rows).sort_values("frac_missing", ascending=False).to_csv(outdir / "feature_missingness_ok_h3.csv", index=False)
    ...
    pd.DataFrame(rows).sort_values(["feature", "method"]).to_csv(outdir / "feature_summary_numeric_by_method_ok_h3.csv", index=False)
    ...
    pd.DataFrame(cat_rows).sort_values(["feature", "method", "value"]).to_csv(outdir / "feature_summary_categorical_by_method_ok_h3.csv", index=False)
...
def spearman_table(
    ...
    for scope, gg in [("pooled", d)] + [(f"method={m}", g) for m, g in d.groupby("method")]:
        ...
        for col in features:
            ...
            rows.append({"scope": scope, "feature": col, "n": int(mask.sum()), "spearman_rho": float(rho), "p_value": float(p)})
...
def build_regression_table(df: pd.DataFrame, ok_h3: pd.Series, outdir: Path) -> pd.DataFrame:
    ...
    bound = (d["bound_state"].astype(str) == "bound").astype(float).to_numpy(dtype=float)
...
def main() -> None:
    ...
    # H3 error column (your RMSD file only has 'error')
    if "h3_error" not in df.columns:
        df["h3_error"] = ""  # treat missing as OK unless RMSD is NaN
    ...
    if "h3_has_glyco_motif" in df.columns:
        df["glyco_motif"] = pd.to_numeric(df["h3_has_glyco_motif"], errors="coerce").map({1.0: "Motif present", 0.0: "Absent"})
    ...
    summarize_feature_stats(
        ...
        categorical_features=["h3_has_glyco_motif"],
        outdir=outdir,
    )
    spearman_table(
        df,
    ...
    pv = df.loc[ok_global, ["id", "method", "rmsd_fv_seqmap"]].pivot_table(index="id", columns="method", values="rmsd_fv_seqmap", aggfunc="first").dropna()
    if "ABodyBuilder2" in pv.columns and "IGFold" in pv.columns:
        d = pv["ABodyBuilder2"] - pv["IGFold"]
        winner = np.where(d < -1e-9, "ABodyBuilder2", np.where(d > 1e-9, "IGFold", "Tie"))
        pd.Series(winner).value_counts().rename_axis("winner").reset_index(name="n_ids").to_csv(
            outdir / "win_counts_global_fv_rmsd.csv", index=False
        )
